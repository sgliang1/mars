...
import com.interstellar.interaction.domain.notification.InteractionNotificationMapper;
 ...
 * 支持幂等：相同 sourceType:actorId:userId:sourceId 的消息只处理一次
 */
@Component
@RocketMQMessageListener(
        topic = MqTopics.NOTIFICATION,
        selectorExpression = MqTopics.TAG_INTERACTION,
        consumerGroup = "interstellar-notification-consumer"
)
public class NotificationConsumer implements RocketMQListener<NotificationMessage> {

    private static final Logger log = LoggerFactory.getLogger(NotificationConsumer.class);
    private static final String IDEMPOTENCY_PREFIX = "interstellar:notif:processed:";
    ...
    public void onMessage(NotificationMessage msg) {
        try {
            ...
            if (!Boolean.TRUE.equals(isNew)) {
                log.debug("通知消息已处理，跳过: {}", idempotencyKey);
                return;
            }
