...
public class AdminAuthService {
    ...
    // 复用 mars-auth 的 Redis key 前缀，避免冲突（admin 用户 id 与 user 用户 id 可能重叠，
    // 但 admin_user 和 user 是独立表，refresh token 按各自前缀隔离）
    private static final String REFRESH_TOKEN_PREFIX = "mars:admin:refresh:";
    private static final String REFRESH_FAMILY_PREFIX = "mars:admin:refresh_family:";
    ...
    public Result<Map<String, Object>> login(String username, String password) {
        ...
        // 存储 refresh token 到 Redis（token family 机制，与 mars-auth 保持一致）
     ...
     * 复用 mars-auth 的 token family 设计
     */
