...
import com.mars.gateway.config.RateLimitProperties;
import com.mars.gateway.config.RateLimitProperties.RateLimit;
...
public class RateLimitFilter implements GlobalFilter, Ordered {
    ...
    private RateLimitResult doRateLimit(String clientIp, String userId, boolean isWrite) {
        try {
            // --- 第一级：IP 维度限流 ---
            String ipKey = "mars:rl:ip:" + clientIp + ":" + (isWrite ? "w" : "r");
            ...
            if (userId != null && !userId.isEmpty()) {
                ...
                String userKey = "mars:rl:user:" + userId + ":" + (isWrite ? "w" : "r");
    ...
    private RateLimit adjustLimitByCredit(String userId, RateLimit base) {
        try {
            String creditKey = "mars:user:credit:" + userId;
