...
import com.mars.post.domain.notification.InteractionNotificationMapper;
    ...
    private void sendPushIfOffline(NotificationMessage msg) {
        try {
            if (!preferenceHelper.shouldPushInteraction(msg.getUserId())) {
                log.debug("用户 {} 互动推送关闭或免打扰，跳过推送", msg.getUserId());
                return;
            }
            ...
            log.debug("互动推送已发送: userId={}, sourceType={}", msg.getUserId(), msg.getSourceType());
        } catch (Exception e) {
