...
public class CommentController {
    ...
    public Result<List<Map<String, Object>>> mine(@RequestHeader("X-User-Id") String userIdStr) {
        ...
        Map<Long, PostEntity> postMap = postMapper.selectBatchIds(postIds).stream()
                .collect(Collectors.toMap(PostEntity::getId, post -> post, (left, right) -> left));

        List<Map<String, Object>> records = comments.stream().map(comment -> {
            PostEntity post = postMap.get(comment.getPostId());
