...
import com.mars.api.RelationFeignClient;
...
/**
 * 社交关系服务（已从 mars-user 提取为独立服务）
 *
 * 跨服务依赖：
 * - follower_count / following_count → 通过 RelationFeignClient 调用 mars-user
 * - follow 通知 → 通过 OutboxService 写入发件箱，异步投递到 mars-interaction
...
public class RelationService {
    ...
    public void follow(Long followerId, Long followedId, String followerName, String sourceType, Long sourceId) {
        ...
        try {
            ...
            // 通过 Feign 更新 mars-user 的粉丝/关注计数
            try {
            ...
            // 通过 Outbox 发送关注通知到 mars-interaction
            try {
